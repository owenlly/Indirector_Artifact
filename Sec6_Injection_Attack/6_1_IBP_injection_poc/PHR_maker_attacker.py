...
def main():
    try:
        ...
        if phr_mode not in ['PHR0', 'PHR1', "DUMMY_PHR"]:
            raise ValueError("Second argument must be 'PHR0', 'PHR1' or 'DUMMY_PHR'")
        input_number = int(sys.argv[3])
    except (IndexError, ValueError) as e:
        print("Usage: python script.py [victim/attacker] [PHR0/PHR1/DUMMY_PHR] [integer]")
        print(f"Error: {e}")
        return
    ...
    with open(filename, 'w') as file:
        ...
        for i in range(BRANCH_NUM-1, end, -1):
            ...
            if (i > 0):
                ...
                if (i == 1) and (phr_mode == "DUMMY_PHR"):
                    file.write(f"\tjmp {person}_set_PHR_end_%+ %1\n\n")
                else:
